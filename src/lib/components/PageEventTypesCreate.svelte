<script lang="ts">
	...
	import { eventTypesCreateTimer, eventTypesTimer } from '$lib/utils/timers/defaults';
		...
		},
		{
			...
			onResult: async (event) => {
				...
				if (eventType === 'success') {
					...
					eventTypesStore.showCreateView = false;
					...
					toast.success(t('eventTypes.create.toast.success.title'), {
						description: t('eventTypes.create.toast.success.description')
					});
				} else {
					toast.error(t('eventTypes.create.toast.error.title'), {
						description: t('eventTypes.create.toast.error.description')
					});
				}
			}
		}
    ...
    eventTypesCreateTimer.start();
	});
</script>

{#if eventTypesCreateTimer.isRunning}
	<div class="mt-8">
	...
	>
		...
		<FormField {form} name="title" let:errors>
			<FormControl let:attrs>
				<PageFormLabel
					label={t('eventTypes.create.form.title.label')}
					description={t('eventTypes.create.form.title.description')}
					isTextWhite={true}
				/>
				<PageFormInput
					...
					{errors}
					{attrs}
					placeholder={t('eventTypes.create.form.title.input.placeholder')}
				/>
			</FormControl>
		...
		<FormField {form} name="color">
			<FormControl>
				<PageFormLabel
					label={t('eventTypes.create.form.color.label')}
					description={t('eventTypes.create.form.color.description')}
					isTextWhite={true}
				/>
		...
		<FormField {form} name="teams">
			<FormControl let:attrs>
				<PageFormLabel
					label={t('eventTypes.create.form.teams.label')}
					description={t('eventTypes.create.form.teams.description')}
					isTextWhite={true}
				/>
				<div class="max-w-sm">
					<div
						class="border-input bg-background flex w-full flex-col items-start gap-2 rounded-md border p-2 focus-within:ring-3 focus-within:ring-blue-600 focus-within:ring-offset-0"
						{...attrs}
					>
						{#if isDisabled}
							<span class="text-muted-foreground">
								{t('eventTypes.create.form.teams.input.placeholder_disabled')}
							</span>
						{:else}
		...
		<Button type="submit" disabled={!$formData.title} class="mt-4 mb-8">
			...
			{t('eventTypes.create.form.button.save.label')}</Button
		>
	</form>
{/if}
